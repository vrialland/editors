    ...
    this.value.forEach(function (elt, i) {
        if (elt && elt.hasValidValue) {
            if (elt.hasValidValue()) {
                ...
                console && console.log('[ArrayEditor Reset] Error encountered on array element ' + i + ', skipping it');
            }
        } else {
            newList.push(elt);
        }
...
/**
 * @return {boolean} true if every item in the array has a valid value
 */
ArrayEditor.prototype.hasValidValue = function () {
    if (this.value === undefined) {
        return false;
    }
    return this.value.every(function (elt) { return !elt.hasValidValue || elt.hasValidValue() });
};
