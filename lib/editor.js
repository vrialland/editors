...
class Editor {
    ...
    createProperties(props, mustBeValid) {
        if (props !== undefined) {
    ...
    hasValidValue() {
        return this.mustBeValid.every(function (name) {
            var res = this[name].hasValidValue();
            if (!res) {
                console && console.log('[Editor hasValidValue] Value of required property "' + name + '" is invalid');
            }
            return res;
        }, this);
    }
    ...
    get errors() {
        var res = {};
        for (var prop of this.propNames) {
            if (this[prop].error != null) {
                res[prop] = this[prop].error;
            }
        }
        return res;
    }
